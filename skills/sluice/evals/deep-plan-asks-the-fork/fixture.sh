...
# and nothing between them and the API. Unlike deep-plan-across-subsystems,
# the README says nothing about where the processes run or what they share, so
# where the counters live is a fork only the partner can settle: the design
# changes shape on the answer, and nothing in the repo gives it.
set -euo pipefail
