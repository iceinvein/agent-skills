#!/usr/bin/env bash
# A deep run stopped exactly where the two stops are already behind it: design
# signed off, plan validated, pre-flight answered, Task 1 landed. What is left
# is execution, which is the part that must not stop.
set -euo pipefail
