...
state left behind, the output written. Aim one at a mock and the most it can
report is that you called the mock the way you meant to. Before a mock stands
in for a dependency, learn what the real one does on the paths your test never
exercises; one that quietly drops a side effect will hold a broken change
green.
...
Three things are worth asking to skip for: generated or scaffolded code, a
config file with no logic in it, and a prototype headed for the bin before
anyone sees it. Ask rather than assume, and treat that list as closed.
