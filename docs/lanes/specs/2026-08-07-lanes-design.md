...
A skill that routes work by change shape into four lanes, each mandating only the
rules that lane needs. It replaces the superpowers plugin's single mandatory
pipeline, which charges every task the cost of the largest task.
...
3. **Non-instructional files shipped inside skill directories (~15k tokens).**
   ...
   superpowers `CLAUDE.md` states the project disagrees with it. `systematic-debugging/`
   ships `CREATION-LOG.md` and four eval fixtures (`test-pressure-1/2/3.md`,
   `test-academic.md`, ~2.9k).
...
One skill directory. The router is the SKILL.md; the five rules are reference
files loaded on demand.

```
skills/lanes/
  SKILL.md          ~600 tok   router: lane table, one-line rules, Lane C exec
...
This is what makes Lane A cheap. Most Lane A work reads the router and nothing
else. It also isolates the expensive, repetitive content (the tables) to the
moment it is actually needed, rather than paying for it on every invocation.

### Lane table

| Lane | Signal | Rules | Typical cost |
|------|--------|-------|--------------|
| 0 | No code change | none — just answer | ~15 tok |
| A | Existing interfaces, one subsystem | test-first, verify | ~600 tok |
| B | New interface, or crosses subsystems | + agree intent, review before merge | ~600 tok |
| C | Several subsystems, or user asks for a plan | + written design and plan | ~600 tok + refs on friction |

**`root-cause` is not lane-assigned.** It is trigger-based and cross-cutting: it
fires on any bug, test failure, or unexpected behaviour, in every lane including
Lane 0. Lanes govern how much process a *change* carries; a bug demands root-cause
investigation whether it surfaced during a one-line fix or a multi-subsystem
build. The other four rules are lane-assigned as tabulated above.

Lane 0 exists to fix a specific superpowers failure: its `using-superpowers`
rationalization table asserts "Questions are tasks. Check for skills," which
routes a pure question into the pipeline.
...
- The agent picks the lane, **states it in one line**, and proceeds. The
  announcement is the escape hatch — it is how the user redirects without being
  asked to confirm.
- Lanes 0, A, and B never stop for approval. Lane C stops for design sign-off
  before code.
- **Escalation is explicit.** If a Lane A task turns out to need a new interface,
  the agent says so and re-routes. Silently finishing in the wrong lane is the
  failure this rule prevents. Downgrading a lane mid-task is not permitted
  without saying so.
- Explicit user instruction always wins. "Just do it" collapses to Lane A
...
"activation": {
  ...
  "default": "global",
  "claudeHookDirective": "Before acting on a request that changes code, pick a lane per the lanes skill and state which one."
}
```
...
Four are lane-assigned; `root-cause` is cross-cutting and trigger-based. Each
...
1. **`intent.md`** — Agree on what is being built before building it. Ask
   ...
   the lane: Lane B agrees in one chat message, Lane C writes it down. No hard
   gate.
...
3. **`root-cause.md`** — Find the root cause before fixing. Three failed fixes
   ...
   by a bug or test failure in any lane, not assigned to one.
...
- **Cannot coexist with superpowers.** The superpowers SessionStart hook mandates
  ...
  the fast path this skill exists to open. Installing `lanes` means disabling
...
4. Lane A invocation cost is under 800 tokens.
5. `bun run skill:audit` passes clean.
6. The agent states its lane on every code-changing request, and re-states it
   when escalating.
...
lane and that it announces — are not covered by either, and this design does not
introduce a harness for them. Routing behaviour will be checked by hand against
the six success criteria above before release.
